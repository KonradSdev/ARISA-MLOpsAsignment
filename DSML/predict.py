...
import shap
import os
from DSML.config import FIGURES_DIR, MODELS_DIR, target, PROCESSED_DATA_DIR, MODEL_NAME
...
def predict(model:CatBoostClassifier, df_pred:pd.DataFrame, params:dict, probs=False)->str|Path:
    ...
    if not probs:
        df_pred[["Patient_ID", target]].to_csv(preds_path, index=False)
    else:
        df_pred[["Patient_ID", target, "predicted_probability"]].to_csv(preds_path, index=False)
...
if __name__=="__main__":
    ...
    with mlflow.start_run(tags={"git_sha": str(git_hash)}):
        ...
        univariate_drift = udc.calculate(analysis_df.drop(columns=["Patient_ID", "prediction", "predicted_probability"], axis=1))
        plot_col_names = analysis_df.drop(columns=["Patient_ID", "prediction", "predicted_probability"], axis=1).columns
